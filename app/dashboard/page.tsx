...
export default function Dashboard() {
  ...
  const { inviteEmail, setInviteEmail, inviteRole, setInviteRole } = useDashboardStore();

  const role = activeMember?.role || (activeOrg as any)?.membership?.role;
  const roleLower = role?.toLowerCase();
  const isOwner = roleLower === 'owner' || roleLower === 'admin' || roleLower === 'org_owner';
  ...
  const { data: rfqs = [], isLoading: loadingRfqs } = useQuery({
    ...
    enabled: !!activeOrg?.id && canManageProcurement,
  });
  ...
  const { data: pendingApprovals = [] } = useQuery({
    ...
    queryFn: () => api.get<any[]>(`/approvals/pending/${activeOrg?.id}`),
    enabled: !!activeOrg?.id && canManageApprovals,
  });
  ...
  if (isActiveOrgPending || isActiveMemberPending) {
    return (
                  ...
                  <div>
                    ...
                    <div className="mt-1 flex gap-2">
                      ...
                      <button
                        ...
                        className="bg-indigo-600 text-white px-4 py-2 rounded-lg text-sm font-bold hover:bg-indigo-700 transition-all disabled:opacity-50"
                      >
